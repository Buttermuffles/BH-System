...
export default function ElectricalBillReceipt() {
    ...
    const printReceipt = () => {
        ...
        const styles = `
            <style>
                @media print { @page { margin: 6mm; size: auto; } }
            ...
            for (let c = 0; c < (printCopies || 1); c++) {
                bodyHtml += `
                    <div class="card">
                        ...
                        <div class="row"><div>Due Date:</div><div>${bill.dueDate ? new Date(bill.dueDate).toLocaleDateString() : ''}</div></div>
        ...
        // allow a moment for styles/images to load
        setTimeout(() => {
            try {
                printWindow.print();
            } catch (e) {
            ...
            // Try to auto-close after printing. Use afterprint where supported and a fallback timeout.
            try {
        ...
        <AuthenticatedLayout>
            ...
            <div className="py-6 print:hidden">
                <div className="mx-auto max-w-7xl sm:px-6 lg:px-8">
                    <div className="mb-8">
                        <h1 className="text-3xl font-bold text-gray-900">Electrical Bill Receipt Calculator</h1>
                        <p className="text-gray-600 mt-2">Calculate electricity bills for multiple rooms</p>
                    </div>
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            <div className="bg-white rounded-xl shadow p-6 border border-gray-200">
                                <div className="flex justify-between items-center mb-4">
                                    ...
                                    <button onClick={addRoom} className="text-sm text-blue-600 hover:text-blue-800 font-medium">+ Add Room</button>
                                </div>
                                <div className="overflow-x-auto">
                                    <table className="min-w-full divide-y divide-gray-200">
                                        <thead>
                                            <tr>
                                                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Room Name</th>
                                                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Previous</th>
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {rooms.map((room) => (
                                                <tr key={room.id}>
                                                    ...
                                                    <td className="px-3 py-2">
                                                        <button onClick={() => removeRoom(room.id)} className="text-red-600 hover:text-red-800">
                                ...
                                <div className="mt-6 flex space-x-3">
                                    <button onClick={calculateBills} className="flex-1 bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700 font-medium">Calculate All</button>
                                    <button onClick={resetForm} className="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 font-medium">Reset</button>
                                </div>
                                </div>
                                <div className="bg-white rounded-xl shadow p-6 border border-gray-200 mt-6">
                                    ...
                                    {calculatedBills.length > 0 ? (
                                        <div className="space-y-4">
                                            <p className="text-sm text-gray-600">Generated {calculatedBills.length} receipts.</p>
                                            <div className="flex items-center gap-3">
                                                <label className="text-sm text-gray-600">Copies:</label>
                                                <select value={printCopies} onChange={(e) => setPrintCopies(parseInt(e.target.value || '1'))} className="px-2 py-1 border rounded text-sm">
                                            ...
                                            <button onClick={printReceipt} className="w-full bg-green-600 text-white py-2 px-4 rounded-lg hover:bg-green-700 font-medium flex items-center justify-center gap-2">
                                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" /></svg>
                                                Print Receipts
                                            </button>
                                            <div className="max-h-[520px] overflow-y-auto border border-gray-100 rounded bg-gray-50 p-4">
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                    {calculatedBills.map((bill, index) => (
                                                        <div key={index} className="bg-white p-5 border border-gray-200 rounded-lg shadow-sm text-sm flex flex-col justify-between min-h-[220px]">
            ...
            {/* Printable Area */}
            <div className="hidden print:block print:p-4 print:text-black">
                <div className="print:grid print:grid-cols-2 print:gap-4">
                    {calculatedBills.map((bill, index) => (
                        <div key={index} className="print:border print:border-gray-800 print:p-4 print:rounded-lg print:break-inside-avoid print:page-break-inside-avoid print:text-sm print:bg-white print:flex print:flex-col print:justify-between print:min-h-[160px]">
                            ...
                            <div className="print:border-t print:border-b print:border-gray-300 print:py-2 print:mb-3 print:space-y-1">
                                <div className="print:flex print:justify-between print:text-sm">
                                    <span>Prev Reading:</span>
                                    <span className="print:prev">{bill.previous ? parseFloat(bill.previous).toLocaleString() + ' kWh' : '-'}</span>
                                </div>
                                <div className="print:flex print:justify-between print:text-sm">
                                    <span>Curr Reading:</span>
                                    <span className="print:curr">{bill.current ? parseFloat(bill.current).toLocaleString() + ' kWh' : '-'}</span>
                                </div>
                                <div className="print:flex print:justify-between print:font-medium print:text-indigo-700">
                                    <span>Consumption:</span>
                                    <span className="print:consumption">{bill.totalKwh != null ? bill.totalKwh.toLocaleString() : '0'} kWh</span>
                                </div>
                                <div className="print:flex print:justify-between print:text-xs print:text-gray-500">
                                    <span>Rate:</span>
                                    <span className="print:rate">{formatCurrency(bill.rate)}/kWh</span>
                                </div>
                            </div>

                            <div className="print:flex print:justify-between print:items-center print:text-lg print:font-bold print:mt-2">
                                <span>TOTAL:</span>
                                <span className="print:total-amt">{formatCurrency(bill.totalAmount)}</span>
                            </div>
